...
class Tensor:
    ...
    def relu(self):
        out = Tensor(np.maximum(np.zeros(shape=self.data.shape), self.data), _op='relu', _children=(self,))
        def _backward():
...
# y = Tensor(value=y)
# y @ x
t1 = MLP(indim=2, outdim=1, hidden_dim=2, n_hidden_layers=1, activation=True)
y = t1(x)
